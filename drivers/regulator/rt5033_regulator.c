...
#ifdef CONFIG_RT5033_SLDO_VBUSDET
	ret = rt5033_set_bits(info->i2c, info->enable_reg,
			info->enable_bit);
#else
	if (info->desc.id==RT5033_ID_LDO_SAFE && chip_rev < RT5033A_REV) {
		/* Safe LDO disable */
...
static int rt5033_regulator_probe(struct platform_device *pdev)
{
	...
	rt5033_set_bits(chip->i2c_client, 0x6b, 0x01);
	usleep(100); /* delay 100 us to wait for normal read (from e-fuse) */
	chip_rev = rt5033_reg_read(chip->i2c_client, 0x03) & 0x0f;
	rt5033_clr_bits(chip->i2c_client, 0x6b, 0x01);
	dev_info(&pdev->dev, "Richtek RT5033 chip_rev = %d)...\n", chip_rev);
